export default function BlendModeControls({
  ...
  const modes = isAdvanced
    ? ["opacity blend", "region overlay", "multiply", "difference", "soft merge", "edge-transfer", "density-transfer", "contour fusion", "pattern crossbreed", "field merge", "palette transfer", "mj blend"]
    : ["opacity blend", "region overlay", "soft merge", "mj blend"];
  ...
  return (
    <article className="inspector-card">
      ...
      {isAdvanced ? (
        <div className="field-row">
          <label>Blend If (Tonal Gating)</label>
          <select value={regionFx.blendIf || "all"} onChange={(e) => setRegionFx((prev) => ({ ...prev, blendIf: e.target.value }))}>
            <option value="all">All tones</option>
            <option value="shadows">Shadows only</option>
            <option value="mids">Midtones only</option>
